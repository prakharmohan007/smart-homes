...
class ReadSyntheticData:
    ...
    def get_image(routine):
        ...
        image = np.zeros((len(routine) * 30, len(routine[0]), 3), np.uint8)
        # print(len(routine[0]))
        ...
        for row in range(len(routine)):
            for col in range(len(routine[row])):
                ...
                image[row * 30:(row + 1) * 30, col] = color
    ...
    @staticmethod
    def get_min_from_time(time_str):
        split_time = time_str.split(sep=':')
    ...
    def read_file(self, file_path):
        ...
        try:
            ...
            print("[ReadSyntheticData] read_file: error reading file. Error: ", err)
            raise
        del data[0]
        ...
        for line in range(len(data)):
            ...
            if line == 0 or prev_day != curr_day:
                routine.append([])
                prev_day = curr_day

            # minute = self.get_min_from_time(split_line[1])
            routine[-1].append(split_line[-1])
    ...
    def read_level(self, level=1, controlled=False, sdp=10, prob=None, num_noise=5):
        ...
        for f_no in range(1, self.num_files + 1):
            ...
            if level == 1:
                file_name = "newsynt_level1_sd" + str(sdp) + "_" + str(f_no)
            elif level == 2:
                file_name = "newsynt_level2_sd" + str(sdp) + "_noise" + str(num_noise) + "_" + str(f_no)
            # elif level == 3:
            else:
                file_name = "newsynt_level3_sd" + str(sdp) + "_prob" + str(prob) + "_" + str(f_no)

            # read parsed file
            file_path = file_dir + file_name + ".csv"
            print("[ReadSyntheticData] read_level: File being read is ", file_path)
            routine = self.read_file(file_path)
            # img, label_img = self.get_image(routine)
            img = self.get_image(routine)

            if IMG_SAVE:
                image_path = self.base_dir + "/raw_images/level" + str(level) + "/" + file_name + ".png"
    ...
    def read_synthetic_data(self, level=1, scale=30, num_days=30,
                            ...
                            base_dir="../../data/synthetic_data/new_synthetic_data/"):
        self.scale = scale
        self.num_days = num_days
        self.num_files = num_files
        self.base_dir = base_dir
...
if __name__ == '__main__':
    ...
    for n in noise:
        for sd in sdp:
            obj.read_synthetic_data(level=2, num_files=10, controlled=True, sdp=sd, num_noise=n)
